...
const registerUser = async (req, res) => {
    ...
    if (userFound) {
        return res.status(409).send({ message: "Utilizator deja existent!" });
    }
...
const login = async (req, res) => {
    ...
    if (!userFound) {
        return res.status(404)
            .send({ message: "Contul nu exista!" });
    }

    const validPass = bcrypt.compareSync(req.body.password, userFound.password);
    if (!validPass) {
        return res.status(400).send({ message: "Parola este gresita!" });
    }
...
const resetPassword = async (req, resp) => {
    ...
    if (!userFound) {
        return resp.status(404).send({ message: "Nu s-a gasit niciun utilizator cu aces username!" })
    } else {
         const salt = bcrypt.genSaltSync(10);
        let ePassword = bcrypt.hashSync(req.body.password, salt);
        await Users.update(
